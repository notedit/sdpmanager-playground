...
const unified_incoming_tracks = new Map()

const planb_incoming_tracks = new Map()
	...
	mngr.on("transport",(transport)=>{
		...
		setTimeout(() => {
			if (protocol === 'plan-b') {
				...
				console.dir(unified_incoming_tracks)
				
				for(let track of unified_incoming_tracks.values()) {
					console.log('create outgoing track', track.getId())
					const outgoingStreamId = "remote-" + track.getId();
					//Get stream
					let outgoingStream = transport.getOutgoingStream(outgoingStreamId);
					//If not found
					if (!outgoingStream)
